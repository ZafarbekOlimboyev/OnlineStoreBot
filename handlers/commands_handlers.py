...
async def new_category_handler(message: Message, state: FSMContext):
    await message.answer("Please, choose category",reply_markup=make_category_list())
...
async def categories(msg: Message):
    await msg.answer("Your categories",reply_markup=make_category_list())
@commands_router.message(Command("new_product"))
async def add_new_product(msg: Message, state: FSMContext):
    await msg.answer(text="Please input product name")
    await state.set_state(CategoryStates.add_product_name)
